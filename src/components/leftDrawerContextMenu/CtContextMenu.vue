<template>
  <q-menu touch-position context-menu>
    <q-list dense>
      <q-item>
        <q-item-section> {{ $t("common.CT") }} '{{ vm.vmid }}' </q-item-section>
      </q-item>
      ...
      <q-item clickable v-close-popup @click="cloneCT(vm.nodeId, vm.vmid)">
        <q-item-section>
          <div>
  ...
  setup(props) {
    ...
    return {
      async doAction(nodeId, vmId, action, opt = {}) {
        try {
          await pve.post(
            `nodes/${nodeId}/lxc/${vmId}/status/${action}`,
            qs.stringify(opt)
          );
          ...
          $q.notify({
            type: "positive",
            position: "top",
            message: `${t("notify.requestSent", { nodeId, vmId })} (${action} CT ${vmId})`,
          });
        } catch (error) {
          console.error(error);
          $q.notify({
